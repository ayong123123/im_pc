...
<template>
  <div class="msglist">
    <ul v-if="searchedChatlist && searchedChatlist.length > 0">
      <li v-for="(item,i) in searchedChatlist" class="sessionlist" :class="[{ active: item.id == msgListStore.activeWindowId && item.chatType == msgListStore.activeTyep },{'offline':item.online == 0}]" @click="selectUserMessage(item)"  @contextmenu.prevent="deleteWindow(item)">
        <div class="list-left">

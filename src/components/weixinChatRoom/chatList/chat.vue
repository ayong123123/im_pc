<template>
  <div class="content">
    <div class="msglist">
      <wx-search></wx-search>
      <wx-userChatList  ref="userChatList"></wx-userChatList>
    </div>
    <div class="chatbox"  >
    ...
    <transition name="showbsox" v-if="chatListStore.ChatRightBoxState">
      <div class="look-right-Box">
        <div class="right-all-user">
          <div class="all-user-count-msg" :style="groupmenberVisible ?{'overflow':'hidden'}:{'overflow':'auto'}" >
          ...
          <ul class="groupmenber" :style="getGroupUsers.length<7 ? {'min-height':'160px'}:''">
            <!--v-show="i.type !=9 && friendsListStore.groupChatInfoList.data.config ==2"-->
            <li v-for="(i,index) in getGroupUsers"
                v-show="index<computedShow"
                :class="{'offline':i.online == 0}"
                v-if="(!deleteUsers && (i.type !=9 && friendsListStore.groupChatInfoList.data.config ==2) || (!deleteUsers && friendsListStore.groupChatInfoList.data.config ==1)) ||(deleteUsers && isBoss && (i.role == 2 || i.role == 3))||(deleteUsers && isManagement && i.role == 3 || (i.type !=9))"
            >
              <label @click="!deleteUsers ? openUserInfo(i):''">
                <el-checkbox label=""  v-model="i.checked" class="check-user-det"  @click="openUserInfo(i)" v-if="deleteUsers"></el-checkbox>
                <div>
                  <!--:class="{doudou:deleteUsers}"-->
                  <img v-lazy="i.head_portrait || moren" alt="" >
                </div>
            ...
            <p class="groupmenber-more" @click="groupmenberVisible = false" v-show="getGroupUsers.length>12 && groupmenberVisible">查看全部群员</p>
            <p class="groupmenber-fold" @click="groupmenberVisible = true" v-show="getGroupUsers.length>12 && !groupmenberVisible">收起</p>
          </ul>
        </div>
        <div class="qunliao-message">
          ...
          <div class="ql-user-edit flex-between" >
            <span>群封面</span>
            <img v-lazy="friendsListStore.groupChatInfoList.data.group_cover" alt="群封面" width="40" height="40" />
          ...
          <div class="ql-user-edit flex-between" @click="editQlRedpackLV()" v-if="isBoss || isManagement">
            <span>最低抢红包等级</span>
            <input type="number" style="width: 100px;" v-if="setRedpackLV" maxlength="16" v-model="friendsListStore.groupChatInfoList.data.grab_level"  @blur="qlRedpackLVBlur(1)" placeholder="设置最低等级">
            <div style="width: 100px;" v-else>
              {{friendsListStore.groupChatInfoList.data.grab_level}}
            </div>
          </div>

          <div class="ql-user-edit flex-between" @click="editSendRedpackLV()" v-if="isBoss || isManagement">
            <span>最低发红包等级</span>
            <input type="number" style="width: 100px;" v-if="setSendRedpackLV" maxlength="16" v-model="friendsListStore.groupChatInfoList.data.send_level"  @blur="qlRedpackLVBlur(2)" placeholder="设置最低等级">
            <div style="width: 100px;" v-else>
              {{friendsListStore.groupChatInfoList.data.send_level}}
            </div>
          </div>

          <div class="ql-user-edit flex-between" v-if="isBoss || isManagement" @click="setModalState('enableSendState')">
            <span>禁言列表</span>
            <img src="@/assets/weChat/right_icon.png" alt="" width="20" />
          </div>
          ...
          <div class="ql-user-edit flex-between" @click="setModalState('searchRecord')" >
            <span>搜索本地聊天记录</span>
          </div>
    ...
    <transition name="showbsox" v-if="chatListStore.ChatRightFriendBoxState">
      <div class="look-right-Box" >
        ...
        <div class="qunliao-message danliao_item" @click="setModalState('searchRecord')">
          <div class="ql-user-edit flex-between">
            <span>搜索本地聊天记录</span>
          </div>
        </div>

        <div class="qunliao-message danliao_item" @click="shareCard">
          <div class="ql-user-edit flex-between">
            <span>把他(她)推荐给朋友</span>
          </div>
        </div>

        <div class="qunliao-message danliao_item" @click="clearHistoryFn">
          <div class="ql-user-edit flex-between">
            <span>清空聊天记录</span>
          </div>
        </div>
        ...
        <div class="clear-message-btn" @click="deleteFriend(friendsListStore.friendInfo)" v-if="friendsListStore.friendInfo.is_admin == 1">
          删除并拉黑
        </div>
      </div>
    </transition>
    ...
    <!--分享名片-->
    <wx-shareCard v-if="shareCardInfo.visible" :shareCardInfo="shareCardInfo" @closeTranspond="closeTranspond"></wx-shareCard>
  </div>
</template>
  ...
  export default {
    ...
    data () {
      return {
        setQlName:false,
        setRedpackLV:false,
        setSendRedpackLV:false,
        setQlNickname:false,
        moren:moren,
        editGongGaoVisible:false,
        deleteUsers:false,
        editRemarkname:false,
        groupmenberVisible:true,
        transpondData:{
          visible:false,
        },
        shareCardInfo:{
    ...
    mounted(){
      // console.log('mergeGroupInfo',this.chatListStore.mergeGroupInfo)
    },
    created (){
...
//          console.log(allArr)
    ...
    watch:{
      ...
      'chatListStore.ChatRightBoxState'() {
          ...
          for(var i in this.msgListStore.friendMsgMap){
            if(this.msgListStore.friendMsgMap[i].msgList){
              for(var v in this.msgListStore.friendMsgMap[i].msgList){
                // this.msgListStore.friendMsgMap[i].msgList[v].code !="3"
...
//                  ("againSenAllTxt",this.msgListStore.friendMsgMap[i].msgList[v]);
    ...
    methods: {
      ...
      //分享名片
      shareCard() {
        const activeWindowId = this.msgListStore.activeWindowId;
        const goodFriendList = [].concat(...this.friendsListStore.goodFriendList);
        //排除导师客服自己
        const friendList =  goodFriendList.filter(item=>item.is_admin ===1 && item.id != activeWindowId)
        const shareList = friendList.concat(...this.friendsListStore.groupChatList)
        this.shareCardInfo.shareList = shareList;

        let temp  = {
          head_portrait:this.friendsListStore.friendInfo.head_portrait,
          id:this.friendsListStore.friendInfo.id,
          phone:this.friendsListStore.friendInfo.phone,
          nickname:this.friendsListStore.friendInfo.nickname,
          username:this.friendsListStore.friendInfo.username,
        }
        let data = {
          create_time:"",
          accept_uid:this.msgListStore.activeWindowId,
          content:JSON.stringify(temp),
          send_uid:this.friendsListStore.friendInfo.id,
          type: this.config.MSG_PRIVATE_CARD,
        }
        //创建转发内容
        this.setZhuanFaMessage(data)
        this.shareCardInfo.visible=true
      },
      closeTranspond() {
        this.transpondData.visible = false;
        this.shareCardInfo.visible = false;
      },
      pictureGroup(e) {
...
//      设置能否添加好友
      ...
      deleteFriend(data){
        ...
        this.$confirm('删除好友不再联系, 是否继续?', '提示', {
          ...
          let deleteItem = this.chatListStore.topInfo.filter(item=>item.id == data.id)[0]
          if(deleteItem.istop) { //如果置顶 则取消置顶
            this.$refs.userChatList.ontop(deleteItem)
          }
        }).catch(() => {
...
//          });
      ...
      //设置最低抢红包等级
      qlRedpackLVBlur(type){
        if(type === 1) { //抢红包
          if(!this.friendsListStore.groupChatInfoList.data.grab_level){
            this.$message.warning('最低抢红包等级不能为空!');
            return
          }
          if(this.friendsListStore.groupChatInfoList.data.grab_level<0) {
            this.$message.warning('不能为负数！');
            return
          }
          this.setRedpackLV = false
          if(this.isManagement || this.isBoss){
            this.setGrabLevel({
              self_uid:this.getUserId,
              group_id:this.msgListStore.activeWindowId,
              grab_level:this.friendsListStore.groupChatInfoList.data.grab_level,
            })
          }
        }else { //发红包
          if(!this.friendsListStore.groupChatInfoList.data.send_level){
            this.$message.warning('最低发红包等级不能为空!');
            return
          }
          if(this.friendsListStore.groupChatInfoList.data.send_level<0) {
            this.$message.warning('不能为负数！');
            return
          }
          this.setSendRedpackLV = false
          if(this.isManagement || this.isBoss){
            this.setSendGrabLevel({
              self_uid:this.getUserId,
              group_id:this.msgListStore.activeWindowId,
              send_level:this.friendsListStore.groupChatInfoList.data.send_level,
            })
          }
        }
      },
...
//          });
    ...
    computed: {
      ...
      computedShow() {
        if((this.isManagement || this.isBoss) && this.groupmenberVisible) {
          return 10
        }
        if((!this.isManagement || !this.isBoss) && this.groupmenberVisible) {
          return 11
        }
        if(!this.groupmenberVisible) {
          return this.getGroupUsers.length
        }
      },
      managementCount(){
        let count = 0
